        ...
        const ordersWithRestaurants = await Promise.all(
          ordersData.map(async (order) => {
            try {
              ...
              const matchingItems = allOrderItems.filter(item => item.order === order.id);
              ...
              const restaurantId = menuItem.restaurant_id || menuItem.restaurant;
          ...
          orders.map((order) => (
            ...
            >
              <View style={styles.orderHeader}>
                <Text style={styles.restaurantName}>{order.restaurant_name || 'Unknown Restaurant'}</Text>
                <Text style={styles.orderTime}>{formatOrderDate(order.created_at)}</Text>
              </View>
              <View style={styles.statusContainer}>
                <View 
                  style={[
                    styles.statusDot,
                    getOrderStatus(order) === 'ready' ? styles.readyStatus : 
                    getOrderStatus(order) === 'preparing' ? styles.preparingStatus : 
                ...
                <Text 
                  style={[
                    styles.statusText,
                    getOrderStatus(order) === 'ready' ? styles.readyText : 
                    getOrderStatus(order) === 'preparing' ? styles.preparingText : 
                    styles.pendingText
                  ]}
                >
                  {getOrderStatus(order) === 'ready' ? 'Ready for pickup' : 
                   getOrderStatus(order) === 'preparing' ? 'Preparing' : 
                   'Order received'}
                </Text>
              </View>
              <View style={styles.orderDetails}>
                <Text style={styles.orderInfo}>{order.total_items} item{order.total_items !== 1 ? 's' : ''}</Text>
                <Text style={styles.orderInfo}>Total: ${order.total_cost.toFixed(2)}</Text>
              </View>
            </TouchableOpacity>
          ))
        )}
