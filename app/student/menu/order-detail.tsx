...
export default function OrderDetailScreen() {
  ...
  useEffect(() => {
    ...
    const formatLocation = (loc: any) => {
      ...
      if (loc.x && loc.y) return `${loc.y.toFixed(6)}, ${loc.x.toFixed(6)}`;
    ...
    const fetchOrderDetails = async (orderId: string) => {
      ...
      try {
        ...
        if (Array.isArray(orderData.order_item) && orderData.order_item.length > 0) {
          for (const oi of orderData.order_item) {
            ...
            processedItems.push({
              ...
              item: {
                ...
                price: itemData?.price || (oi.line_total / (oi.quantity || 1)),
        ...
        if (Array.isArray(orderData.order_item) && orderData.order_item.length > 0) {
          ...
          if (firstItem.item) {
            ...
            const itemData = Array.isArray(firstItem.item) ? firstItem.item[0] : firstItem.item;
            if (itemData && itemData.restaurant) {
              // Handle both array and direct object formats for restaurant
              restaurant = Array.isArray(itemData.restaurant) ? itemData.restaurant[0] : itemData.restaurant;
            }
          }
        }
        ...
        if (orderData.pickup_window) {
          // Handle both array and direct object formats
          const pickupWindowData = Array.isArray(orderData.pickup_window) 
          ...
          if (pickupWindowData) {
            ...
            if (pickupWindowData.collection_point) {
              const collectionPointData = Array.isArray(pickupWindowData.collection_point)
        ...
        const formattedRestaurant = restaurant
          ? {
              ...
              location: restaurant.location ? formatLocation(restaurant.location) : 'Not specified',
            }
          : undefined;
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        <View style={styles.orderCard}>
          ...
          <Text style={styles.restaurantName}>{order.restaurant_info?.name || 'Unknown Restaurant'}</Text>
          <Text style={styles.orderTime}>Ordered: {formatOrderDate(order.created_at)}</Text>
          ...
          <View style={styles.statusContainer}>
            <Text 
              style={[
                styles.statusText, 
                getOrderStatus(order) === 'ready' ? styles.readyStatus : styles.preparingStatus
              ]}
            >
              {getOrderStatus(order) === 'ready' ? 'Ready for pickup' : 'Preparing'}
            </Text>
          </View>
          ...
          {orderItems.map((orderItem) => (
            <View key={orderItem.id} style={styles.itemRow}>
              <Text style={styles.itemName}>{orderItem.quantity}x {orderItem.item?.name || 'Unknown Item'}</Text>
              <Text style={styles.itemPrice}>${orderItem.line_total.toFixed(2)}</Text>
            </View>
          ))}
          <View style={styles.totalRow}>
            <Text style={styles.totalLabel}>Total</Text>
            <Text style={styles.totalAmount}>${order.total_cost.toFixed(2)}</Text>
          </View>
          ...
          {order.pickup_window ? (
            <Text style={styles.pickupTimeInfo}>
              Pickup Window: {formatOrderDate(order.pickup_window.open)} - {formatOrderDate(order.pickup_window.close)}
            </Text>
          ) : (
            <Text style={styles.pickupTimeInfo}>
          ...
          {getOrderStatus(order) === 'collected' ? (
            <View style={styles.collectedContainer}>
              <Text style={styles.collectedText}>Order has been collected</Text>
              <Text style={styles.collectedTime}>Collected at: {formatOrderDate(order.collected_at || '')}</Text>
            </View>
          ) : order.pickup_pin ? (
            <View style={styles.pinContainer}>
              <Text style={styles.pinLabel}>Your PIN:</Text>
              <Text style={styles.pinCode}>{order.pickup_pin}</Text>
              <Text style={styles.pinInstructions}>Show this code to the restaurant staff when collecting your order</Text>
              
              <View style={styles.collectButton}>
                <Button 
                  title={collecting ? "Processing..." : "Mark as Collected"}
            ...
            <View>
              ...
              <Button 
                title={generatingPin ? "Generating..." : "Generate Pickup PIN"}
                onPress={handleGeneratePin}
                disabled={generatingPin || getOrderStatus(order) !== 'ready'}
                color="#00BFA6"
              />
              {getOrderStatus(order) !== 'ready' && (
                <Text style={styles.pinWarning}>PIN can only be generated when order is ready for pickup</Text>
              )}
            </View>
          )}
        </View>
